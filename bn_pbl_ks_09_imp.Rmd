...
#zrobić współwydanie
#przygotowanie linku w formularzu, żeby przenosiło na stronę konkretnej książki !!! - kontakt z Matysiakiem


test <- out %>%
  select(za_opis_wspoltworcow) %>%
  filter(grepl("\\|",za_opis_wspoltworcow))
...
x <- 1:(length(polaczone)-1)
for (i in x) {
  progress(match(i,x), max.value = length(x))
  polaczone$za_adnotacje <- ifelse(grepl("<U\\+(....)>",polaczone[,i]),
                                   ifelse(nchar(polaczone$za_adnotacje)!=0,paste(polaczone$za_adnotacje,paste("UWAGA! Błąd kodowania w polu ",as.character(names(polaczone[i]))," Szukaj frazy \"???\"",sep = ""),sep = "# "),paste("UWAGA! Błąd kodowania w polu ",as.character(names(polaczone[i]))," Szukaj frazy \"???\"",sep = "")),as.character(polaczone$za_adnotacje))
...
for (i in x) {
  ...
  polaczone$za_adnotacje <- ifelse(grepl("\\$",polaczone[,i]),
                                   ifelse(nchar(polaczone$za_adnotacje)==0,paste("UWAGA! Ze względu na błędny zapis BN w polu ",as.character(names(polaczone[i]))," wydrukowano znak \"$\". Przeredaguj treść pola.",sep = ""),paste(polaczone$za_adnotacje,paste("UWAGA! Ze względu na błędny zapis BN w polu ",as.character(names(polaczone[i]))," wydrukowano znak \"$\". Przeredaguj treść pola.",sep = ""),sep = "# ")),as.character(polaczone$za_adnotacje))
}
...
for (i in x) {
  ...
  final <- out %>%
    ...
    select(-podzial)
  write.csv2(final, paste("C:/Users/Cezary/Desktop/2009_podmiotowa_do_importu",i,".csv",sep = ""), row.names = F, na = '', fileEncoding = 'UTF-8')
}
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.csv2(final, paste("C:/Users/Cezary/Desktop/2009_przedmiotowa_do_importu",i,".csv",sep = ""), row.names = F, na = '', fileEncoding = 'UTF-8')
}
```
...
for (i in x) {
  ...
  final <- out %>%
    filter(podzial %in% ile[[i]]) %>%
    select(-podzial)
  write.csv2(final, paste("C:/Users/Cezary/Desktop/2009_antologie_do_importu",i,".csv",sep = ""), row.names = F, na = '', fileEncoding = 'UTF-8')
}
```
