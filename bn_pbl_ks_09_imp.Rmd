...
bn_ok <- bn_ok %>%
  ...
  select(id, X008, X009, X015, X041, X044, X080, X100, X245, X246, X250, X260, X300, X380, X386, X490, X500, X501, X546, X600, X650, X655, X700, X710, X800, X830, rok, osoba_bn_autor, osoba_bn_temat, dziedzina_PBL, bez_ukd_ale_PBL, czy_wspomnienia_reportaz, slowa_literackie, id_grupy_dubli)
...
```
```{r problemy}
#wprawdzić długość pól w bazie i wydrukować informację, jeśli fraza jest zbyt długa dla oracle'a - to już jest w ostatnim chunku: przycinanie zbyt długich komórek -- zająć się TYM, trzeba dać to wcześniej i naprawić, bo źle dodrukowuje do adnotacji
#zastosować uwagi z githuba
#sprawdzić, czy generowanie list do nadpisania nie powinno zostać w jakikolwiek sposób uwzględnione
#zrobić współwydanie
...
for (i in x) {
  ...
  out$za_adnotacje <- ifelse(dlugosc<nchar(as.character(out[,i])),
                             ifelse(za_adnotacje!="",paste(out$za_adnotacje,paste("UWAGA! Pole ", as.character(names(out[i]))," było zbyt długie i zostało przycięte. Zredaguj treść pola.",sep = ""),sep = "# "),paste("UWAGA! Pole ", as.character(names(out[i]))," było zbyt długie i zostało przycięte. Zredaguj treść pola.",sep = "")),as.character(out$za_adnotacje))
