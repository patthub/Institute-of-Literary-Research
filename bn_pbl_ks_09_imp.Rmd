...
bn_ks <- bn_ks %>%
  mutate(dziedzina_PBL = ifelse(str_detect(X080,"\\$a80[^\\d]")|str_detect(X080,"\\$a82"),"ukd_lit",
                                ifelse(str_detect(X080,"\\$a791")|str_detect(X080,"\\$a792")|str_detect(X080,"\\$a7\\.09"),"ukd_tfrtv",
                                       ifelse(str_detect(X080,"\\$a01")|str_detect(X080,"\\$a050")|str_detect(X080,"\\$a070"),"ukd_biblio",
                                              ifelse(str_detect(X080,"\\$a002")|str_detect(X080,"\\$a305")|str_detect(X080,"\\$a39")|str_detect(X080,"\\$a929[^\\.]"),"ukd_pogranicze","bez_ukd_PBL")))))
  
#wczytanie wyselekcjonowanych gatunków literackich na bazie UKD
...
gatunki_lit_2 <- gatunki_lit %>%
  filter(pole=="X650") %>%
  select(1)
#szukanie po wybranych gatunkach literackich dla ukd literackiego
bez_UKD_lita <- sqldf("select *
                          from bn_ks a
                          join gatunki_lit_1 b on a.X655 like ('%'||b.gatunki||'%')") %>%
  unique() %>%
  filter(dziedzina_PBL == "bez_ukd_PBL") %>%
  select(id) %>%
  mutate(bez_ukd_ale_gatunek = "bez_ukd_lit")
bez_UKD_litb <- sqldf("select *
                          from bn_ks a
  ...
  select(id) %>%
  mutate(bez_ukd_ale_gatunek = "bez_ukd_lit")
#wczytanie wyselekcjonowanych gatunków tfrtv na bazie UKD
...
gatunki_tfrtv_2 <- gatunki_tfrtv %>%
  filter(pole=="X650") %>%
  select(1)
#szukanie po wybranych gatunkach tfrtv dla ukd literackiego
bez_UKD_tfrtv_a <- sqldf("select *
  ...
  filter(dziedzina_PBL == "bez_ukd_PBL") %>%
  select(id) %>%
  mutate(bez_ukd_ale_gatunek = "bez_ukd_tfrtv")
...
bez_UKD_tfrtv_b <- sqldf("select *
  ...
  select(id) %>%
  mutate(bez_ukd_ale_gatunek = "bez_ukd_tfrtv")
#połączenie plików bez ukd, ale z gatunkami
bez_UKD <- rbind(bez_UKD_lita,bez_UKD_litb,bez_UKD_tfrtv_a,bez_UKD_tfrtv_b) %>%
  unique() %>%
  arrange(id) %>%
  group_by(id) %>%
  mutate(bez_ukd_ale_gatunek = paste(bez_ukd_ale_gatunek,collapse = "|")) %>%
  ungroup() %>%
  unique()

bn_ks <- bn_ks %>%
  full_join(.,bez_UKD,by = "id") %>%
  filter(!is.na(id))

#wspomnienia,pamiętniki,literatura podróżnicza
wspomnienia <- bn_ks %>%
  mutate(czy_wspomnienia = ifelse(str_detect(X655,"Pamiętniki i wspomnienia")|str_detect(X655,"Literatura podróżnicza")|str_detect(X655,"Pamiętniki"),"tak",NA)) %>%
  select(id,czy_wspomnienia) %>%
  unique()
bn_ks <- bn_ks %>%
  full_join(.,wspomnienia,by = "id")

#słowa literackie w polu 245, 650 i 655
...
bn_ok <- bn_ks %>%
  filter(!(is.na(osoba_pbl_dzial_id_name)&dziedzina_PBL=="bez_ukd_PBL"&is.na(bez_ukd_ale_gatunek)&is.na(czy_wspomnienia)&is.na(slowa_literackie)))
...
bn_ok <- bn_ok %>%
  ...
  select(id, X008, X041, X044, X080, X100, X245, X246, X250, X260, X300, X380, X386, X490, X500, X501, X500, X546, X600, X650, X655, X700, X830, rok, osoba_pbl_dzial_id_name, dziedzina_PBL, bez_ukd_ale_gatunek, czy_wspomnienia, slowa_literackie, czy_polonik) %>%
  unique()
...
#KP działa z listą rekordów, które nie mają UKD, ale mają gatunki - intuicja jest taka, że lista jest zbyt długa, a gatunki brane za szeroko (gatunki zbudował jeszcze stary wybór UKD)
```
