...
###przypisanie autorów podmiotowej do literatur i przypisanie do redaktorów na podstawie literatury z pola 655
podmiotowa_redaktorzy <- bn_ok %>%
  filter(rodzaj_ksiazki=="podmiotowa") %>%
  filter(id %notin% do_podmiotowej1$id) %>%
...
#zbudować obiekt z literami literatury polskiej na kształt pbl_literatury_obce i dać drugi left join w poniższm sqldf; to samo zrobić dla 650
...
do_podmiotowej3 <- marc_field_655[!duplicated(marc_field_655$same),] %>%
  ...
  mutate(TW_TWORCA_ID = NA,
         ...
         TW_IMIE = NA) %>%
  select(names(marc_field_600))
###przypisanie autorów podmiotowej do literatur i przypisanie do redaktorów na podstawie literatury z pola 650
podmiotowa_redaktorzy <- bn_ok %>%
  filter(rodzaj_ksiazki=="podmiotowa") %>%
  filter(id %notin% do_podmiotowej1$id) %>%
  filter(id %notin% do_podmiotowej2$id) %>%
