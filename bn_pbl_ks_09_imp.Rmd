...
options(scipen = 999)
pacman::p_load(utf8,googlesheets4,zoo,stringr,splitstackshape,plyr,dplyr,sqldf,stringdist,fuzzyjoin,data.table,svMisc,tidyverse,RJDBC,combinat,gtools)
...
count <- as.data.frame(table(bn_ok$dziedzina_PBL,bn_ok$rodzaj_ksiazki))
colnames(count) <- c("dziedzina","czy automatycznie","ile")
...
poprzednie_importy <- rbind(listy_2004,listy_2005,listy_2006,listy_2007,listy_2008) %>%
  ...
  mutate(ZA_STATUS_IMP = paste(unique(ZA_STATUS_IMP),collapse = "|")) %>%
  ungroup() %>%
  unique() %>%
  ungroup() %>%
...
poprzednie_importy <- poprzednie_importy %>%
  mutate(ile = str_count(poprzednie_importy$X650,"\\|")+1)
###Najszybsze permutacje na świecie!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#TUTAJ
library(arrangements)
i <- 432
test <- as.data.frame(permutations(unlist(str_split(poprzednie_importy$X650[i],"\\|")), str_count(poprzednie_importy$X650[i],"\\|")+1))

test <- unite(data = test, col =  "test",sep = "|")

###########



test <- unlist(str_split(poprzednie_importy$X650[i],"\\|"))
test <- append(test, "1")

a <- as.data.frame(test[c(1,8)])
b <- test[c(2,8)]


test <- as.data.frame(permutations(str_count(poprzednie_importy$X650[i],"\\|")+1,str_count(poprzednie_importy$X650[i],"\\|")+1,unlist(str_split(poprzednie_importy$X650[i],"\\|")))) %>%
...
for (i in x){
  progress(match(i,x), max.value = length(x)) 
    if (is.na(poprzednie_importy$X650[i])&!is.na(poprzednie_importy$X655[i])){
      combinations <- as.data.frame(permn(unlist(str_split(poprzednie_importy$X655[i],"\\|")))) %>%
      summarize_each(funs(paste(., collapse = "|"))) %>%
      data.table::transpose(.) %>%
        mutate(X650 = NA,
               X655 = V1,
        ...
        select(id,X650,X655)
      do_dzialowania <- rbind(do_dzialowania,combinations)
    } else if (is.na(poprzednie_importy$X655[i])&!is.na(poprzednie_importy$X650[i])){
      combinations <- as.data.frame(permn(unlist(str_split(poprzednie_importy$X650[i],"\\|")))) %>%
        summarize_each(funs(paste(., collapse = "|"))) %>%
        data.table::transpose(.) %>%
        mutate(X650 = V1,
               X655 = NA,
      ...
      combinations <- as.data.frame(permn(unlist(str_split(poprzednie_importy$X650[i],"\\|")))) %>%
        ...
        mutate(X650 = V1,
               id = poprzednie_importy$id[i]) %>%
        select(id,X650)
      combinations650 <- data.frame(stringsAsFactors = FALSE)
      combinations650 <- rbind(combinations650,combinations)
      combinations <- as.data.frame(permn(unlist(str_split(poprzednie_importy$X655[i],"\\|")))) %>%
        summarize_each(funs(paste(., collapse = "|"))) %>%
        data.table::transpose(.) %>%
        mutate(X655 = V1,
               id = poprzednie_importy$id[i]) %>%
        select(id,X655)
      combinations655 <- data.frame(stringsAsFactors = FALSE)
      combinations655 <- rbind(combinations655,combinations)
      combinations <- merge(combinations650,combinations655,by = "id",all = TRUE) %>%
        select(id,X650,X655)
      do_dzialowania <- rbind(do_dzialowania,combinations)
    }
}
do_dzialowania[] <- lapply(do_dzialowania, gsub, pattern='NA', replacement=NA)



if (is.na(poprzednie_importy$X650)&!is.na(poprzednie_importy$X655)) {
  x <- poprzednie_importy$id[is.na(poprzednie_importy$X650)&!is.na(poprzednie_importy$X655)]
  for (i in x) {
    progress(match(i,x), max.value = length(x)) 
    combinations <- as.data.frame(permn(unlist(str_split(poprzednie_importy$X655[i],"\\|")))) %>%
      summarize_each(funs(paste(., collapse = "|"))) %>%
...
} else if (is.na(poprzednie_importy$X655)&!is.na(poprzednie_importy$X650)) {
    x <- poprzednie_importy$id[is.na(poprzednie_importy$X655)&!is.na(poprzednie_importy$X650)]
    for (i in x) {
    progress(match(i,x), max.value = length(x)) 
    combinations <- as.data.frame(permn(unlist(str_split(poprzednie_importy$X650[i],"\\|")))) %>%
      summarize_each(funs(paste(., collapse = "|"))) %>%
  ...
  select(ZA_STATUS_IMP,joint,DZ_DZIAL_ID,DZ_NAZWA) %>%
  mutate(joint = str_remove_all(str_replace_all(joint,"(\\$a)","\\\\#7\\1"),"#")) %>%
  group_by(joint) %>%
  mutate(grupa = paste(unique(ZA_STATUS_IMP),collapse = ","),
         DZ_DZIAL_ID = paste(unique(DZ_DZIAL_ID),collapse = "|"),
         DZ_NAZWA = paste(unique(DZ_NAZWA),collapse = "|")) %>%
  ungroup() %>%
  filter(!grepl("\\|",DZ_DZIAL_ID)) %>%
  filter(grepl("IOK",grupa)) %>%
  filter(!grepl("INO|IMP|IPN|IPC",grupa)) %>%
  filter(!grepl("osobowe",DZ_NAZWA))
  #jak zmiksować te kombinacje????

combinations <- data.table::transpose(as.data.frame(permn(unlist(str_split(poprzednie_importy$joint[2],"\\|"))))) %>%
  group_by_at(vars(V1:V3)) %>%
  summarize_all(paste, collapse=",")

  
  select(joint, DZ_DZIAL_ID, DZ_NAZWA, grupa) %>%
  unique() %>%
  mutate(ile = str_count(grupa,",")+1) %>%
  filter(grepl("IOK",grupa)) %>%
  filter(!grepl("INO|IMP|IPN|IPC",grupa)) %>%
