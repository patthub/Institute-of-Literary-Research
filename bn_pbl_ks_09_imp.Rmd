...
###TUTAJ###
###coś tu nie gra, grepl nie działa
gatunki_podmiotowe <- str_replace_all(paste(gatunki_podmiotowe$X655,collapse = "|"),"(.{2})(\\$a)","\\2")
bn_ok$czy_podmiotowy <- grepl(gatunki_podmiotowe,bn_ok$X655)
test2 <- grepl(test,bn_ok$X655)
grepl("instance|percentage", Text)
gatunki_bez_ukd <- sqldf("select a.id
                         from bn_ok a
                         inner join gatunki_podmiotowe b on a.X655 like ('%'||'b.X655'||'%')")

count <- as.data.frame(table(test$rodzaj_ksiazki))
