...
gatunki_bn <- sqldf("select *
                    from gatunki_bn a
                    left join gatunki_pbl b on lower(a.X655) like '%'||b.gatunek||'%'")
gatunki_bn <- sqldf("select *
                      from gatunki_bn a
                      left join gatunki_pbl b on lower(a.X650) like '%'||b.gatunek||'%'")
...
za_jezyk_oryginalu <- marc_field_041 %>%
  select(ZA_UWAGI,ZA_JEZYK_ORYGINALU = `$a`) %>%
...
`$a` = paste(ifelse(is.na(`$a`),"",as.character(`$a`)),collapse = " ")
