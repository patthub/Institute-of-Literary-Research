...
for (i in x) {
  ...
  test2$czy_nazwisko[i] <- grepl(test2$bn_nazwisko[i],test2$X245[i])
  test2$czy_imie[i] <- grepl(test2$bn_imie[i],test2$X245[i])
...
marc_field <- bn_ks %>%
  ...
  mutate(X245 = str_remove_all(X245,"^\\|"),
         indicator = str_replace_all(X245,"(^.*?)(\\$.*)","\\1"))
subfield_list<- str_extract_all(bn_ks$X245,"\\$.")
subfield_list<- unique(unlist(subfield_list))
...
BN_autor <- bn_ks %>%
  ...
  mutate(X245c = marc_field$`$c`)
autor1 <- BN_autor %>%
  filter(X700=="") %>%
  mutate(drugi_autor = "")
BN_autor <- BN_autor %>%
  cSplit(.,"X700",sep = "|",direction = "long") %>%
  mutate(czy_funkcja = grepl("\\$e|\\$b",X700),
         czy_t = grepl("\\$t",X700),
         drugi_autor = ifelse(czy_funkcja==FALSE&czy_t==FALSE, as.character(X700),"")) %>%
  select(-8,-7)
BN_wspoltworca <- BN_autor

BN_autor <- BN_autor %>%
  ddply(., .(licznik, X100, X245, X245c, X501), summarize, X700 = paste(X700, collapse="|"), drugi_autor = paste(drugi_autor, collapse = "|")) %>%
  mutate(drugi_autor = str_remove(drugi_autor,"^\\|+"),
         drugi_autor = str_replace_all(drugi_autor,"\\|+","\\|"),
         drugi_autor = ifelse(drugi_autor=="",NA,as.character(drugi_autor))) %>%
  arrange(licznik,drugi_autor) %>%
  select(names(autor1)) %>%
  union(.,autor1) %>%
  arrange(licznik) %>%
  mutate(drugi_autor = ifelse(is.na(drugi_autor),"",as.character(drugi_autor)),
         drugi_autor = ifelse(is.na(X245c),"",as.character(drugi_autor))) %>%
  group_by(licznik) %>%
  mutate(X100 = paste(X100,drugi_autor,sep = "|")) %>%
  ungroup() %>%
  mutate(X100 = str_remove(X100,"^\\|+"),
         X100 = str_remove(X100,"\\|+$")) %>%
  select(-length(.))
...
BN_autor <- BN_autor %>%
  mutate(czy = str_detect(X245c,osoba_nazw)|str_detect(X245c,osoba_imie)) %>%
  rename(ZA_ADNOTACJE = X245c) %>%
  mutate(ZA_ADNOTACJE = ifelse(is.na(ZA_ADNOTACJE),"",as.character(ZA_ADNOTACJE)),
         ###tu jest jakiś problem
         ZA_ADNOTACJE = ifelse(czy==FALSE,paste("Uwaga, błąd w danych osobowych", ZA_ADNOTACJE, sep = ": "),"")) %>%
  select(licznik,X100,ZA_ADNOTACJE)

marc_field <- BN_autor %>%
  select(licznik,X100) %>%
  mutate(X100 = str_remove_all(marc_field$X100,"^\\|"),
...
BN_autor <- BN_autor %>%
  ...
  mutate(X100 = marc_field$`$a`,
         X100 = str_remove(X100,"(?<=[a-zaáàâãäăāåąæeéèêëěēėęiíìîïīįioóòôõöőøœuúùûüűūůyýcćčçdďđđgģğkķlłļnńñňņŋrřsśšşsßtťŧþţzżźž?])(\\.$)"))
...
test <- bn_ok %>%
  mutate(#X500 = ifelse(grepl("\\$aTyt[,\\.] oryg",X500),"",as.character(X500)),
         ...
         tylko_podmiotowe = ifelse(str_count(X655,"\\$a")>0&str_count(X655,"\\$a")==str_count(X655,"\\$y"),"tak","nie"))
...
test2 <- test2 %>%
  ...
  mutate(czy_tytul_obcy = ifelse(is.na(jezyk),"nie dotyczy",
                                 ifelse(!grepl("pol",jezyk),
                                        ifelse(X246!="","ok",
                                               ifelse(grepl("Tyt[,\\.] oryg",X501),"ok","problem")),"nie dotyczy")))
###usunąć te, które nie mają tytułów oryginalnych
count <- as.data.frame(table(test2$czy_tytul_obcy))
...
# przedmiotowe; osoba z PBL jako temat
#600
marc_field_600 <- bn_ok %>%
  select(id,X600)%>%
  filter(X600!="") %>%
  mutate(X600=str_replace_all(X600,"(^|\\|)","~\\1")) %>%
  cSplit(.,"X600",sep = "~",direction = "long") %>%
  ...
  mutate(X600=str_remove_all(X600,"^\\|")) %>%
  mutate(indicator = str_replace_all(X600,"(^.*?)(\\$.*)","\\1"))
subfield_list<- str_extract_all(bn_ok$X600,"\\$.")
subfield_list<- unique(unlist(subfield_list))
empty_table<- data.frame(matrix(ncol = length(subfield_list),nrow = lengths(marc_field_600)[1]))
colnames(empty_table) <-subfield_list
marc_field_600<-cbind(marc_field_600,empty_table)
...
test_przedm$czy_gatunek <- grepl(gatunki_podmiotowe,test_przedm$X655)
test_przedm2 <- test_przedm %>%
...
###
count <- as.data.frame(table(test$ile_gatunkow))
