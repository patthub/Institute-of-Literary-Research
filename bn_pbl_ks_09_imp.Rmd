...
do_przedmiotowej1 <- marc_field_600
...
do_przedmiotowej2 <- rbind(marc_field_600,marc_field_655,marc_field_650)
...
deskryptory655 <- as.data.frame(table(poprzednie_importy$X655,poprzednie_importy$pracownik_pbl)) %>%
  filter(Freq>0) %>%
  arrange(Var1)

deskryptory655$id_grupy <- cumsum(!duplicated(deskryptory655[1]))
deskryptory655 <- deskryptory655[!duplicated(deskryptory655$id_grupy),] %>%
  select(-id_grupy) %>%
  arrange(-Freq) %>%
  mutate(Var1 = str_remove_all(Var1,"\\\\7\\$a"),
         Var1 = str_remove_all(Var1,"\\$2DBN"),
         Var1 = str_replace_all(Var1,"\\$.|\\|"," "),
         Var1 = str_replace_all(Var1," ",".*"))
x <- 1:length(deskryptory655$Var1)
test <- data.frame(stringsAsFactors = FALSE)
for (i in x) {
  progress(match(i,x), max.value = length(x))
    do_przedmiotowej3 <- przedmiotowa_redaktorzy %>%
      mutate(osoba = ifelse(grepl(deskryptory655$Var1[i],X655),as.character(deskryptory655$Var2)[i],NA),
             ile = ifelse(grepl(deskryptory655$Var1[i],X655),as.integer(deskryptory655$Freq)[i],NA),
             co = ifelse(grepl(deskryptory655$Var1[i],X655),as.character(deskryptory655$Var1)[i],NA)) %>%
      filter(!is.na(osoba))
    if (length(do_przedmiotowej3$osoba)>0) {
...
do_przedmiotowej3 <- do_przedmiotowej3[!duplicated(do_przedmiotowej3$id_grupy),] %>%
  mutate(TW_TWORCA_ID = NA,
         TW_NAZWISKO = NA,
         TW_IMIE = NA)
  select(id, redaktor_dzialu = osoba)

do_przedmiotowej <- rbind(do_przedmiotowej,do_przedmiotowej2,do_przedmiotowej3)
