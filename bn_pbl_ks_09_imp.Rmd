...
###TUTAJ - sprawdzić na 2010 czy to wszystko działa
...
test <- test %>% 
   left_join(.,marc_field_100 %>% unite("BN_osoba",`$a`:`$c`,sep = " ",na.rm = TRUE) %>% select(tw_tworca_id,BN_osoba),by="tw_tworca_id") %>% 
  unique() %>% 
  select(TW_TWORCA_ID = tw_tworca_id,TW_NAZWISKO,TW_IMIE,BN_osoba)
