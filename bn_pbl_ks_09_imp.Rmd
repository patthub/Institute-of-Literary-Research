...
count <- as.data.frame(table(test$porownanie))

test <- merge(test,count,by.x = "porownanie",by.y = "Var1",all.x = TRUE) %>%
...
test2 <- test %>%
  ...
  mutate(Freq = paste(unique(Freq),collapse = "|"),
         id = paste(unique(id),collapse = "|"),
         porownanie_2 = paste(unique(porownanie_2),collapse = "|"),
         porownanie_3 = paste(unique(porownanie_3),collapse = "|")) %>%
  ungroup() %>%
  unique()
##

test2 <- test$porownanie_2 %>%
  unique()
test2 <- as.vector(test2)

test2 <- c("ola","jola")

permutations <- as.data.frame(permutations(test2,2))
permutations$distance <- adist(permutations$V1,permutations$V2)


x <- 1:length(permutations$V1)
for (i in x) {
  progress(match(i,x), max.value = length(x)) 
  permutations$distance[i] <- adist(permutations$V1[i],permutations$V2[i])
}

adist("abalatonmel","abalatommel") == 1
#TUTAJ!!!
...
bn_ok <- bn_ok %>%
  mutate(rodzaj_ksiazki = ifelse(str_count(X245, " / ")+1>2,"antologia",
         ...
         rodzaj_ksiazki = ifelse(ilu_tworcow>4,"antologia",as.character(rodzaj_ksiazki))) %>%
...
```{r usunięcie dubli książek}
test <- bn_ok %>%
  select(id,X245,X260,X300,X830)
#X245
marc_field_245 <- test %>%
  select(id,X245)%>%
  filter(X245!="") %>%
...
subfield_list<- str_extract_all(test$X245,"\\$.")
subfield_list<- unique(unlist(subfield_list))
...
marc_field_245 <- marc_field_245 %>%
  ...
  mutate(`$a` = paste(`$a`,collapse = " "),
         `$b` = paste(ifelse(is.na(`$b`),"",`$b`),collapse = " ")) %>%
  ungroup() %>%
  unique() %>%
  mutate(X245ab = str_to_lower(str_replace_all(paste(`$a`,`$b`,sep = ""), "\\W", ""))) %>%
  select(id,X245ab)

#X300
test <- test %>%
  mutate(X300 = str_remove_all(X300,", \\[\\d+\\]")) %>%
  left_join(.,marc_field_245,by="id") %>%
  mutate(total = paste(X245ab,str_to_lower(str_replace_all(X260, "\\W", "")),str_to_lower(str_replace_all(X300, "\\W", "")),str_to_lower(str_replace_all(X830, "\\W", "")),sep = "|")) %>%
  select(id,total) %>%
  arrange(total)
...
test <- test %>%
  left_join(.,bn_ok,by="id")

write.csv2(test, "C:/Users/Cezary/Desktop/KP_sprawdza.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
...
```
```{r problemy}
#spróbować zająć się książkami zdublowanymi
#czy do opisu fizycznego dodać uwagi z 501? - nie, raczej nei dodawać
#wsbogacić 500 i 546 o wystąpienia fraz dotycz oraz pol, case sensitive = false; upewnić się, że jeśli jest kilka $a, to wchodzi tylko to, które ma interesujące nas słowo, nic więcej
#wprawdzić długość pól w bazie i wydrukować informację, jeśli fraza jest zbyt długa dla oracle'a
#co zrobić, jeśli książka ma więcej, niż 4 osoby w 100? -> jeśli jest podmiotowa, to zrobić antologię, a wszystkie osoby przenieść do adnotacji; jeśli jest inna niż podmiotowa, to ok, bo będzie tylu autorów
...
za_opis_wspoltworcow <- BN_wspoltworca %>%
  ...
  filter(!is.na(OS_NAZWISKO)) %>%
  mutate(OS_IMIE = ifelse(OS_IMIE=="*","",as.character(OS_IMIE)),
         opis = paste(funkcja,OS_IMIE, OS_NAZWISKO, sep = " "),
...
marc_field_500 <- marc_field_500 %>%
  filter(!grepl("oryg(\\.|\\,)",X500)&grepl("pseud|nazwa",X500)) %>%
  mutate(`$a` = str_remove(`$a`," \\;+$| \\:+$"))
...
BN_wydawnictwo <- data %>%
  ...
  ddply(., .(ZA_UWAGI), summarize, WY_WYDAWNICTWO_ID = paste(WY_WYDAWNICTWO_ID, collapse="|"), WY_NAZWA = paste(WY_NAZWA, collapse="|"), WY_MIASTO = paste(WY_MIASTO, collapse="|"), za_rok_wydania = paste(ifelse(is.na(za_rok_wydania),"",as.integer(za_rok_wydania)), collapse="|"))

#połączenie wszystkich elementów w jedną tabelę
kolejnosc <- c("ZA_UWAGI","RZ_NAZWA","ZA_RO_ROK","ZA_TYPE","RZ_RODZAJ_ID","DZ_NAZWA","DZ_DZIAL_ID","TW_TWORCA_ID","AM_AUTOR_ID","AM_NAZWISKO","AM_IMIE","ZA_TYTUL","ZA_TYTUL_ORYGINALU","ZA_JEZYK_ORYGINALU","OS_OSOBA_ID","OS_NAZWISKO","OS_IMIE","fo_symbol","opis","wydanie","za_tomy","instytucja","WY_WYDAWNICTWO_ID","WY_MIASTO","WY_NAZWA","za_rok_wydania","za_opis_ks","seria","TW_NAZWISKO","TW_IMIE","redaktor_dzialu","ZA_ADNOTACJE")
polaczone <- data %>%
  select(ZA_UWAGI) %>%
...
write.csv2(out, "C:/Users/Cezary/Desktop/2009_podmiotowa_do_importu.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
#test <- out %>%
#  mutate(rekord_BN = ifelse(rekord_BN=="",NA,as.character(rekord_BN))) %>%
#  fill(rekord_BN) %>%
#  left_join(.,automatyczny_IOK,by="rekord_BN") %>%
#  mutate(rekord_BN = as.integer(rekord_BN)) %>%
#  left_join(.,data %>% select(1:26,42), by=c("rekord_BN"="ZA_UWAGI"))
#write.csv2(test, "C:/Users/Cezary/Desktop/2009_imp_test.csv", row.names = F, na = '', fileEncoding = 'UTF-8')



#pomyśleć o pętli, która będzie przygotowywała pliki po 2000 wierszy
#zrobić przedmiotowe, antologie i współwydania
#przejrzeć github i nanieść korekty
```

```{r książki przedmiotowe}
#przeredagować skrypt z podmiotowej
data <- bn_ok %>%
  filter(rodzaj_ksiazki=="przedmiotowa")
#uwolnienie kolumn z danymi z bn i przetworzenie do PBL
#1: za_uwagi, rz_nazwa, za_ro_rok, za_type, rz_rodzaj_id, tw_tworca_id, tw_nazwisko, tw_imie, dz_dzial_id, dz_nazwa, redaktor_dzialu
...
write.csv2(out, "C:/Users/Cezary/Desktop/2009_przedmiotowa_do_importu.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
#test <- out %>%
#  mutate(rekord_BN = ifelse(rekord_BN=="",NA,as.character(rekord_BN))) %>%
#  fill(rekord_BN) %>%
#  left_join(.,automatyczny_IOK,by="rekord_BN") %>%
...
write.csv2(out, "C:/Users/Cezary/Desktop/2009_antologie_do_importu.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
#test <- out %>%
#  mutate(rekord_BN = ifelse(rekord_BN=="",NA,as.character(rekord_BN))) %>%
#  fill(rekord_BN) %>%
#  left_join(.,automatyczny_IOK,by="rekord_BN") %>%
...
```{r sprawdzić, czy coś się przyda}
---
title: "R Notebook"
output: html_notebook
---
...
gatunki_pbl <- data.frame(gatunki = c("aforyzm", "album", "antologia", "autobiografia", "dziennik", "esej", "felieton", "inne", "kazanie", "list", "miniatura prozą", "opowiadanie", "poemat", "powieść", "proza", "proza poetycka", "reportaż", "rozmyślanie religijne", "rysunek, obraz", "scenariusz", "szkic", "tekst biblijny", "tekst dramatyczny", "wiersz", "wspomnienia", "wypowiedź"))
