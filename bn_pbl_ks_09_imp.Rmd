...
```{r przypisanie redaktorów do przedmiotowej i podmiotowej}
...
PBL_hasla_osobowe <- PBL_dzialy_path %>%
  filter(grepl("osobow",DZ_NAZWA)) %>%
...
reczne <- data.frame(DZ_DZIAL_ID = c(697,697,697,697,825,1990,1187,1454,884,902,544,544,544,544,544,792,697,1133,544),DZ_NAZWA = c("Hasła osobowe (brytyjska i irlandzka)","Hasła osobowe (brytyjska i irlandzka)","Hasła osobowe (brytyjska i irlandzka)","Hasła osobowe (brytyjska i irlandzka)","Hasła osobowe (grecka starożytna)","Hasła osobowe (łacińska średniowieczna)","Hasła osobowe (syryjska)","Hasła osobowe (esperanto)","Hasła osobowe (holenderska)","Hasła osobowe (Indii)","Hasła osobowe (Afryki Subsaharyjskiej)","Hasła osobowe (Afryki Subsaharyjskiej)","Hasła osobowe (Afryki Subsaharyjskiej)","Hasła osobowe (Afryki Subsaharyjskiej)","Hasła osobowe (Afryki Subsaharyjskiej)", "Hasła osobowe (egipsko-arabska)", "Hasła osobowe (brytyjska i irlandzka)","Hasła osobowe (palestyńsko-arabska)","Hasła osobowe (Afryki Subsaharyjskiej)"), redaktor_dzialu = c("BEATAK","BEATAK","BEATAK","BEATAK","BEATAS","BEATAS","BEATAD","CEZARY","TOMASZU","EWA","EWA","EWA","EWA","EWA","EWA","BEATAD","BEATAK","BEATAD","EWA"), nazwa = c("angielsk","szkock","irlandzk","walijsk","greck","łacińsk","syryjsk","esperanck","niderlandzk","indyjsk","południowoafryka","senegalsk","nigeryjsk","afrykańsk","ruandyjsk","egipsk. nowożytn","celtyck","palestyńsk","somalijsk"))

PBL_hasla_osobowe <- rbind(PBL_hasla_osobowe,reczne)
...
marc_field_655$same <- cumsum(!duplicated(marc_field_655[1]))
do_podmiotowej3 <- marc_field_655[!duplicated(marc_field_655$same),] %>%
...
marc_field_650$same <- cumsum(!duplicated(marc_field_650[1]))
do_podmiotowej4 <- marc_field_650[!duplicated(marc_field_650$same),] %>%
  select(id,DZ_DZIAL_ID,DZ_NAZWA,TW_NAZWISKO,TW_IMIE,redaktor_dzialu) %>%
  filter(!is.na(DZ_DZIAL_ID)) %>%
  mutate(TW_TWORCA_ID = NA) %>%
  select(names(marc_field_600))
...
###zbudować słownik literatur z UKD i przeanalizować pole 080



###przypisanie autorów podmiotowej do literat literatury polskiej i przypisanie do redaktorów na podstawie literatury z pola 655
