...
options(scipen = 999)
pacman::p_load(zoo,openxlsx,stringr,splitstackshape,plyr,dplyr,sqldf,stringdist,fuzzyjoin,data.table,svMisc,tidyverse,RJDBC,rvest,RSelenium,jsonlite)
...
wzorcowe_bn <- data.frame(stringsAsFactors=FALSE)
...
for (i in x) {
  ...
  url_2 <- tolower(str_replace_all(pbl_tworcy$TW_NAZWISKO," ", "+"))[1796]
  url_3 <- "+"
  url_4 <- tolower(str_replace_all(pbl_tworcy$TW_IMIE," ","+"))[1796]
  ...
  tryCatch({
    ...
    select(bn_id=id,bn_name=name)
                        ...
                        bn <<- data.frame(bn_id = "brak danych (CR)", bn_name = "brak danych (CR)")
    })
...
write.csv2(wzorcowe_bn, file = "C:/Users/Cezary/Desktop/tworcy_pbl_wzorcowe_bn.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
