...
2. Marc tools -> MarcBreaker i change from .mrc to .mrk (30 seconds per a text file that can be uploaded to R)
...
b <- sprintf("%02d", i-1)
path1 <- "C:/Users/User/bn_all_1.10.2019/msplit000000"
path2 <- b
path3 <- ".mrk"
...
remove(bn1)
data1 <- mutate(data1,
                czy = grepl("^\\=LDR",data1$bn1))
data1 <- data1 %>%
  filter(bn1!="")

data1 <- mutate(data1,
                id = ifelse(grepl("^\\=001",data1$bn1),str_replace_all(data1$bn1,"(^\\=001 .*?\\w)(.*?)($)","\\2"),NA))
data1 <- data1 %>%
  fill(id) 
data1$id[data1$czy==TRUE] <- NA
data1 <- data1 %>%
  fill(id,.direction = "up") %>%
  select(-2)
data1 <- mutate(data1,
                field = str_replace_all(data1$bn1, "(=)(\\w{3})(.*)", "\\2"),
                content = str_replace_all(data1$bn1,"(=)(\\w{3})(\\s{2})(.*)","\\4"))
data1 <- data1 %>%
  select(2,3,4)
data1 <- mutate(data1,
