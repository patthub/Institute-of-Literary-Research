...
X610 <- pbl_books %>%
  ...
  mutate(X610 = trim(paste("24$a",KH_NAZWA,sep = ""))) %>%
...
X611 <- pbl_books %>%
  ...
  mutate(X611 = trim(paste("24$a",KH_NAZWA,sep = ""))) %>%
...
X630 <- pbl_books %>%
  ...
  mutate(X630 = trim(paste("\\4$a",KH_NAZWA,sep = ""))) %>%
...
X650 <- pbl_books %>%
  ...
  mutate(X650 = trim(ifelse(MARC_FIELD=="4.65",paste("04$a",KH_NAZWA,sep = ""),
                       ifelse(MARC_FIELD=="24.65",paste("24$a",KH_NAZWA,sep = ""),"")))) %>%
...
X651 <- pbl_books %>%
  ...
  mutate(X651 = trim(paste("\\4$a",KH_NAZWA,sep = ""))) %>%
...
long <- reshape(pbl_marc_books,
  ...
  write.table(., "pbl_marc.csv", append = FALSE, sep = "  ", dec = ",", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
pbl_articles1 <- dbGetQuery(PBL,
                        "select z.za_zapis_id \"rekord_id\", z.za_type \"typ\", rz.rz_rodzaj_id \"rodzaj_zapisu_id\", rz.rz_nazwa \"rodzaj_zapisu\", 
                          ...
                          z.za_zrodlo_rok \"rok\", z.za_zrodlo_nr \"numer\", z.za_zrodlo_str \"strony\",z.za_tytul_oryginalu,z.za_te_teatr_id,z.ZA_UZYTK_WPIS_DATA,z.ZA_UZYTK_MOD_DATA
                        from pbl_zapisy z
                        join IBL_OWNER.pbl_zapisy_tworcy zt on zt.zatw_za_zapis_id=z.za_zapis_id
                        join IBL_OWNER.pbl_tworcy tw on zt.zatw_tw_tworca_id=tw.tw_tworca_id
                        join IBL_OWNER.pbl_zapisy_autorzy za on za.zaam_za_zapis_id=z.za_zapis_id
...
X005 <- pbl_articles %>%
  ...
  unique() %>%
  rename(X005 = ZA_UZYTK_MOD_DATA) %>%
  mutate(X005 = str_replace(X005,"(\\d{4})(-)(\\d{2})(-)(\\d{2})(.*$)","\\1\\3\\5"))
...
X100 <- pbl_articles %>%
  ...
  mutate(X700 = ifelse(grepl("\\|",X100),str_replace(X100,"^(.*?)(\\|)(.*$)","\\3"),NA),
         ...
         X700 = ifelse(X700==X100,NA,as.character(X700)))
#X240
X240 <- pbl_articles %>%
  select(rekord_id,ZA_TYTUL_ORYGINALU) %>%
  unique() %>%
  rename(X240 = ZA_TYTUL_ORYGINALU) %>%
  mutate(X240 = ifelse(is.na(X240),"",paste("\\\\$a",str_replace(X240,"^(.*?\\])(.*$)","\\1"),sep = "")))
...
X520 <- pbl_articles %>%
  ...
  unique() %>%
  rename(X520 = adnotacja) %>%
  mutate(X520 = ifelse(is.na(X520),"",paste("2\\$a",X520,sep = "")))
...
X600 <- pbl_articles %>%
  ...
  mutate(tworca_nazwisko = ifelse(!is.na(tworca_nazwisko)&grepl(",",tworca_nazwisko)&tworca_imie=="*",str_replace_all(tworca_nazwisko,",","|"),as.character(tworca_nazwisko)),
         tworca_imie = ifelse(!is.na(tworca_nazwisko)&tworca_imie=="*","",as.character(tworca_imie))) %>%
  cSplit(.,"tworca_nazwisko",sep = "|",direction = "long") %>%
  mutate(X600 = ifelse(tworca_imie!="",paste("14$a",tworca_nazwisko,", ",tworca_imie,sep = ""),paste("14$a",tworca_nazwisko,sep = ""))) %>%
  select(rekord_id,X600) %>%
  group_by(rekord_id) %>%
  mutate(X600 = paste(X600,collapse = "|")) %>%
  ungroup() %>%
...
X610 <- pbl_articles %>%
  ...
  unique() %>% 
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="24.61"),by = c("HP_NAZWA","KH_NAZWA")) %>%
  mutate(X610 = paste("24$a",KH_NAZWA,sep = "")) %>%
  select(rekord_id,X610) %>%
  group_by(rekord_id) %>%
  mutate(X610 = paste(X610,collapse = "|")) %>%
  ungroup() %>%
...
X611 <- pbl_articles %>%
  ...
  unique() %>% 
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="24.611"),by = c("HP_NAZWA","KH_NAZWA")) %>%
  mutate(X611 = paste("24$a",KH_NAZWA,sep = "")) %>%
  select(rekord_id,X611) %>%
  group_by(rekord_id) %>%
  mutate(X611 = paste(X611,collapse = "|")) %>%
  ungroup() %>%
...
X630 <- pbl_articles %>%
  ...
  unique() %>% 
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="4.63"),by = c("HP_NAZWA","KH_NAZWA")) %>%
  mutate(X630 = paste("\\4$a",KH_NAZWA,sep = "")) %>%
  select(rekord_id,X630) %>%
  group_by(rekord_id) %>%
  mutate(X630 = paste(X630,collapse = "|")) %>%
  ungroup() %>%
...
X650 <- pbl_articles %>%
  ...
  unique() %>% 
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="4.65"|MARC_FIELD=="24.65"),by = c("HP_NAZWA","KH_NAZWA")) %>%
  mutate(X650 = ifelse(MARC_FIELD=="4.65",paste("04$a",KH_NAZWA,sep = ""),
                       ifelse(MARC_FIELD=="24.65",paste("24$a",KH_NAZWA,sep = ""),""))) %>%
  select(rekord_id,X650) %>%
  group_by(rekord_id) %>%
  mutate(X650 = paste(X650,collapse = "|")) %>%
...
X651 <- pbl_articles %>%
  ...
  unique() %>% 
  inner_join(.,pbl_hp %>% filter(MARC_FIELD=="4.651"),by = c("HP_NAZWA","KH_NAZWA")) %>%
  mutate(X651 = paste("\\4$a",KH_NAZWA,sep = "")) %>%
  select(rekord_id,X651) %>%
  group_by(rekord_id) %>%
  mutate(X651 = paste(X651,collapse = "|")) %>%
  ungroup() %>%
...
X655 <- pbl_articles %>%
  ...
  unique() %>%
  group_by(rekord_id) %>%
  mutate(X655 = paste(unlist(str_extract_all(tytul,"(?<!^.)(?<=\\[)(.*?)(?=\\])")),collapse = "|")) %>%
  ungroup() %>%
  select(rekord_id,X655) %>%
  cSplit(.,"X655",sep = "|",direction = "long") %>%
  mutate(X655 = paste("\\4$a",X655,sep = "")) %>%
...
X700 <- pbl_articles %>%
  select(rekord_id,funkcja_osoby,wspoltworca_nazwisko,wspoltworca_imie) %>%
  unique() %>%
  mutate(X700 = ifelse(is.na(wspoltworca_nazwisko),"",paste("1\\$a",wspoltworca_nazwisko,", ",wspoltworca_imie,"$e",funkcja_osoby,sep = ""))) %>%
  select(rekord_id,X700) %>%
  rbind(X100 %>% select(rekord_id,X700) %>% cSplit(.,"X700",sep = "|",direction = "long"),.) %>%
  arrange(rekord_id) %>%
  filter(!is.na(X700)&X700!="") %>%
...
X710 <- pbl_articles %>%
  ...
  unique() %>%
  rename(X710 = ZA_INSTYTUCJA) %>%
  mutate(X710 = ifelse(is.na(X710),NA,paste("24$a",X710,"$4isp",sep = "")))

#773
X773 <- pbl_articles %>%
  select(rekord_id,czasopismo,rok,numer,strony) %>%
  unique() %>%
  mutate(X773 = paste("0\\$t",czasopismo,"$gR. ",rok,", ",numer,", ",strony,sep = "")) %>%
...
X964 <- pbl_articles %>%
  ...
  unique() %>%
  rename(X964 = dzial) %>%
  mutate(X964 = str_replace(X964,"(.*)( - .$)","\\1"))
