...
X100 <- pbl_books %>%
  ...
  mutate(X700 = ifelse(grepl("\\|",X100),str_replace(X100,"^(.*?)(\\|)(.*$)","\\3"),NA),
         ...
         razem = ifelse(!is.na(X700),paste(X100,X700,sep = "|"),as.character(X100)))
#X245
#10$aMasaż ironią :$bwybór fraszek i aforyzmów /$cZbigniew Kurzyński ; wstępem opatrzył Stanisław Stanik ; [il. Piotr Szałkowski].
X245 <- merge(pbl_books,X100,by = "rekord_id",all = TRUE) %>%
  select(rekord_id,autor_nazwisko,autor_imie,tytul,wspoltworcy,razem) %>%
  unique() %>%
  mutate(tytul_ok = ifelse(substr(tytul,nchar(tytul),nchar(tytul))=="]",trim(str_replace(tytul,"(^.*?)(\\[.*?$)","\\1")),
                           ifelse(substr(tytul,1,1)=="[",trim(str_replace(tytul,"(^.*?\\])(.*?$)","\\2")),as.character(tytul))),
         autor = ifelse(!is.na(razem),str_replace_all(str_remove_all(razem," ; \\$4aut"),"(..)(\\$a)","\\2"),NA))
