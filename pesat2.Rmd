...
#X008
X008 <- pbl_books %>%
  select(rekord_id,ZA_UZYTK_WPIS_DATA) %>%
  unique() %>%
  rename(X008 = ZA_UZYTK_WPIS_DATA) %>%
  mutate(X008 = paste(str_replace(X008,"(\\d{2})(\\d{2})(-)(\\d{2})(-)(\\d{2})(.*$)","\\2\\4\\6"),"                              -d",sep = ""))
...
X250 <- pbl_books %>%
  ...
  mutate(X250 = ifelse(is.na(X250),"",paste("$a",as.character(X250),sep = "")))
...
X264 <- pbl_books %>%
  ...
  ungroup() %>%
  unique() %>%
  mutate(X264 = paste(X264,",$c",ZA_RO_ROK,sep = "")) %>%
...
X490 <- pbl_books %>%
  ...
  mutate(X490 = ifelse(is.na(X490_2)&!is.na(X490_1),paste("0 $a",X490_1,sep = ""),
...
class(X655$gatunek)
  ...
  mutate(gatunek = paste(,collapse = "|")) %>%
  ungroup()

  mutate(tytul_ok = ifelse(substr(tytul,nchar(tytul),nchar(tytul))=="]",trim(str_replace(tytul,"(^.*?)(([\\.!?] {0,1})(\\[))(.*?$)","\\5")),""))
...
test <- pbl_hp %>%
  ...
  unique()
#TUTAJ#
#na koniec zrobić trim(na wszystkich kolumnach)
