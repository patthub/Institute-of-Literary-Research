...
bn_record_people <- bn_record_people %>%
  mutate(id_osoby = seq.int(nrow(bn_record_people)))
...
cz_record_people <- cz_record_people %>%
  mutate(id_osoby = seq.int(nrow(cz_record_people)))
names_of_columns <- colnames(cz_record_people)

cz_record_people <- merge(cz_record_people,test,by = "name",all = TRUE)
cz_record_people <- cz_record_people %>%
...
#do zrobienia
#field X100
marc_field <- bn_articles %>%
  select(id,X100)%>%
  filter(X100!="")
subfield_list<- str_extract_all(bn_articles$X100,"\\%.")
subfield_list<- unique(unlist(subfield_list))
empty_table<- data.frame(matrix(ncol = length(subfield_list),nrow = lengths(marc_field)[1]))
colnames(empty_table) <-subfield_list
marc_field<-cbind(marc_field,empty_table)
...
bn_record_people_art_X100 <- marc_field %>%
  select(record_id = id, `%1`, `%2`, `%d`, `%4`, `%3`, `%s`,`%w`,`%p`,`%k`) %>%
  mutate(`%1` = paste(`%1`,`%3`,sep = " ")) %>%
  mutate(`%2` = paste(`%2`,`%4`,sep = " ")) %>%
  select(-`%3`,-`%4`) %>%
  mutate(data_set = "bn_articles", field = "X100") %>%
  select(data_set, field, 1:8)
bn_record_people_art_X100[] <- lapply(bn_record_people_art_X100, gsub, pattern=' NA', replacement='')
bn_record_people_art_X100[] <- lapply(bn_record_people_art_X100, gsub, pattern='NANA', replacement='')
bn_record_people_art_X100[] <- lapply(bn_record_people_art_X100, gsub, pattern='NA$', replacement='')
bn_record_people_art_X100$name <- paste(bn_record_people_art_X100$`%1`,bn_record_people_art_X100$`%2`,bn_record_people_art_X100$`%d`,sep = "|")
bn_record_people_art_X100[] <- lapply(bn_record_people_art_X100, gsub, pattern='\\|NA', replacement='')
#field X700
marc_field <- bn_articles %>%
  select(id,X700)%>%
  filter(X700!="")
subfield_list<- str_extract_all(bn_articles$X700,"\\%.")
subfield_list<- unique(unlist(subfield_list))
empty_table<- data.frame(matrix(ncol = length(subfield_list),nrow = lengths(marc_field)[1]))
colnames(empty_table) <-subfield_list
marc_field<-cbind(marc_field,empty_table)
...
bn_record_people_art_X700 <- marc_field %>%
  select(record_id = id, `%1`, `%2`, `%d`, `%4`, `%3`, `%s`,`%w`,`%p`,`%k`) %>%
  mutate(`%1` = paste(`%1`,`%3`,sep = " ")) %>%
  mutate(`%2` = paste(`%2`,`%4`,sep = " ")) %>%
  select(-`%3`,-`%4`) %>%
  mutate(data_set = "bn_articles", field = "X700") %>%
  select(data_set, field, 1:8)
bn_record_people_art_X700[] <- lapply(bn_record_people_art_X700, gsub, pattern=' NA', replacement='')
bn_record_people_art_X700[] <- lapply(bn_record_people_art_X700, gsub, pattern='NANA', replacement='')
bn_record_people_art_X700[] <- lapply(bn_record_people_art_X700, gsub, pattern='NA$', replacement='')
bn_record_people_art_X700$name <- paste(bn_record_people_art_X700$`%1`,bn_record_people_art_X700$`%2`,bn_record_people_art_X700$`%d`,sep = "|")
bn_record_people_art_X700[] <- lapply(bn_record_people_art_X700, gsub, pattern='\\|NA', replacement='')
#field X701
marc_field <- bn_articles %>%
  select(id,X701)%>%
  filter(X701!="")
subfield_list<- str_extract_all(bn_articles$X701,"\\%.")
subfield_list<- unique(unlist(subfield_list))
empty_table<- data.frame(matrix(ncol = length(subfield_list),nrow = lengths(marc_field)[1]))
colnames(empty_table) <-subfield_list
marc_field<-cbind(marc_field,empty_table)
...
bn_record_people_art_X701 <- marc_field %>%
  select(record_id = id, `%1`, `%2`, `%d`, `%3`, `%s`, `%p`) %>%
  mutate(`%4` = NA, `%w` = NA, `%k` = NA) %>%
  mutate(`%1` = paste(`%1`,`%3`,sep = " ")) %>%
  mutate(`%2` = paste(`%2`,`%4`,sep = " ")) %>%
  select(-`%3`,-`%4`) %>%
  mutate(data_set = "bn_articles", field = "X701") %>%
  select(colnames(bn_record_people_art_X100)[1:10])
bn_record_people_art_X701[] <- lapply(bn_record_people_art_X701, gsub, pattern=' NA', replacement='')
bn_record_people_art_X701[] <- lapply(bn_record_people_art_X701, gsub, pattern='NANA', replacement='')
bn_record_people_art_X701[] <- lapply(bn_record_people_art_X701, gsub, pattern='NA$', replacement='')
bn_record_people_art_X701$name <- paste(bn_record_people_art_X701$`%1`,bn_record_people_art_X701$`%2`,bn_record_people_art_X701$`%d`,sep = "|")
bn_record_people_art_X701[] <- lapply(bn_record_people_art_X701, gsub, pattern='\\|NA', replacement='')

#concatenate bn

bn_record_people_art <- rbind(bn_record_people_art_X100,bn_record_people_art_X700,bn_record_people_art_X701) 
bn_record_people_art <- bn_record_people_art %>%
  mutate(id_osoby = seq.int(nrow(bn_record_people_art)))

names_of_columns <- colnames(bn_record_people_art)

set1 <- bn_record_people_art %>%
  select(1:6,11:12) %>%
  mutate(`%1` = paste(`%1`,`%2`,`%d`, sep = " ")) %>% 
  select(data_set,field,record_id,`%1`,name,id_osoby)
set1 <- set1 %>%
...
set2 <- bn_record_people_art[c(1:3,7,11:12)] %>%
  filter(!is.na(`%s`))
set2 <- set2 %>%
  mutate(subfield = colnames(set2)[4])
colnames(set2)[4] <- "name_form"
set3 <- bn_record_people_art[c(1:3,8,11:12)] %>%
  filter(!is.na(`%w`))
set3 <- set3 %>%
  mutate(subfield = colnames(set3)[4])
colnames(set3)[4] <- "name_form"
set4 <- bn_record_people_art[c(1:3,9,11:12)] %>%
  filter(!is.na(`%p`))
set4 <- set4 %>%
  mutate(subfield = colnames(set4)[4])
colnames(set4)[4] <- "name_form"
set5 <- bn_record_people_art[c(1:3,10,11:12)] %>%
  filter(!is.na(`%k`))
set5 <- set5 %>%
  mutate(subfield = colnames(set5)[4])
colnames(set5)[4] <- "name_form"

bn_record_people_art <- rbind(set1,set2,set3,set4,set5)
bn_record_people_art <- bn_record_people_art %>%
  select(data_set,record_id,field,subfield,id_osoby,name_form,id_osoby) %>%
  mutate(name_form_id = paste(data_set,record_id,id_osoby,field,subfield,sep = "-")) %>%
  mutate(name_simple = str_replace_all(str_to_lower(name_form), "\\W", ""))
...
#putting it together
record_people <- rbind(bn_record_people,cz_record_people)

tab_2 <- record_people %>%
  select(name_form,name_form_id,name_simple,person_appearance_id) %>%
  ...
  mutate(person_appearance_id = paste(unique(person_appearance_id), collapse = "|")) %>%
  unique() %>%
  mutate(w_obu_zbiorach = grepl("bn",name_form_id)&grepl("cz",name_form_id))
