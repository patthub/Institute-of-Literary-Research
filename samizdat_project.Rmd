...
bn_people_record_id_X100 <- marc_field %>%
  select(record_id = id, last_name = `%1`, first_name = `%2`, name2 = `%4`, name3 = `%3`, years = `%d`) %>%
  mutate(last_name = paste(last_name,name3,sep = " ")) %>%
  mutate(first_name = paste(first_name,name2,sep = " "))
bn_people_record_id_X100[] <- lapply(bn_people_record_id_X100, gsub, pattern=' NA', replacement='')
bn_people_record_id_X100[] <- lapply(bn_people_record_id_X100, gsub, pattern='NANA', replacement='')
bn_people_record_id_X100[] <- lapply(bn_people_record_id_X100, gsub, pattern='NA$', replacement='')
...
bn_people_record_id_X100 <- bn_people_record_id_X100 %>%
  ...
  mutate(record_id_group = paste("bn_X100",record_id_group,sep = "|")) %>%
  ungroup() %>%
...
bn_people_record_id_X700[] <- lapply(bn_people_record_id_X700, gsub, pattern='NANA', replacement='')
bn_people_record_id_X700[] <- lapply(bn_people_record_id_X700, gsub, pattern='NA$', replacement='')
...
bn_people_record_id_X700 <- bn_people_record_id_X700 %>%
  group_by(name_total) %>%
  mutate(record_id_group = paste(record_id,collapse = "|")) %>%
  mutate(record_id_group = paste("bn_X700",record_id_group,sep = "|")) %>%
  ungroup() %>%
  select(last_name,first_name,years,name_total,record_id_group) %>%
  unique()
...
marc_field <- bn_books %>%
  select(id,X701)%>%
  filter(X701!="")
marc_field$X701<-str_replace_all(marc_field$X701,"(^|\\|)","#\\1")
marc_field<- cSplit(marc_field,"X701",sep = "#",direction = "long")
marc_field<- marc_field%>%
  filter(X701!="")
marc_field$X701<-str_remove_all(marc_field$X701,"^\\|")
...
bn_people_record_id_X701 <- marc_field %>%
  select(record_id = id, last_name = `%1`, first_name = `%2`, name3 = `%3`, years = `%d`) %>%
  mutate(last_name = paste(last_name,name3,sep = " "))
bn_people_record_id_X701[] <- lapply(bn_people_record_id_X701, gsub, pattern=' NA', replacement='')
bn_people_record_id_X701[] <- lapply(bn_people_record_id_X701, gsub, pattern='NANA', replacement='')
bn_people_record_id_X701[] <- lapply(bn_people_record_id_X701, gsub, pattern='NA$', replacement='')
...
write.csv2(people_record_id, "C:/Users/Cezary/Desktop/people_record_id.csv", row.names = F, na = '', fileEncoding = 'UTF-8')
```
