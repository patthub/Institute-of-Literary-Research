...
test <- bn_record_people %>%
  select(name) %>%
  unique() %>%
  ungroup()
test <- test %>%
  mutate(person_appearance_id = paste("bnb",seq.int(nrow(test)),sep = ""))

bn_record_people <- merge(bn_record_people,test,by = "name",all = TRUE)
bn_record_people <- bn_record_people %>%
  select(names_of_columns,person_appearance_id) %>%
  arrange(field,as.integer(record_id))

set1 <- bn_record_people %>%
  select(1:6,11:12) %>%
  mutate(`%1` = paste(`%1`,`%2`,`%d`, sep = " ")) %>% 
  select(data_set,field,record_id,`%1`,name,person_appearance_id)
set1 <- set1 %>%
...
bn_record_people <- bn_record_people %>%
  select(data_set,record_id,field,subfield,person_appearance_id,name_form) %>%
  mutate(name_form_id = paste(data_set,record_id,person_appearance_id,field,subfield,sep = "-")) %>%
...
test <- cz_record_people %>%
  select(name) %>%
  unique()
test <- test %>%
  mutate(person_appearance_id = paste("czb",seq.int(nrow(test)),sep = ""))
...
cz_record_people <- cz_record_people %>%
  ...
  select(data_set,field,record_id,name_form,name,person_appearance_id)
cz_record_people <- cz_record_people %>%
  mutate(subfield = "$$a")
